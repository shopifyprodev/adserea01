...
app.prepare().then(async () => {
  ...
  router.post("/api/import-product1", koaBody(), async (ctx) => {
    ...
    var options = {
        method: 'GET',
        url: 'https://magic-aliexpress1.p.rapidapi.com/api/product/1005001548085474',
        headers: {
      ...
      await axios.request(options).then(function (response) {
        ...
        //   var itemvariants = response.data.skuProperties[0].skuPropertyValues;

        //   itemvariants.map(async (itemsv) => {

        //    console.log("itt var ",  itemsv.propertyValueDisplayName)
        ...
        //   });
         ...
         // ctx.body = response.data.skuList;
...
router.post("/api/import-product", koaBody(), async (ctx) => {
   ...
   var interval = 8000;
    ...
    const Dget = await GetPostdata.forEach(async function loop(el, index)  {
        setTimeout(async function () {
            ...
            if(!flag){                
                const ResGet = await axios.request(options).then(function (response) {
                    if(response.data.name!='No product found in Aliexpress API, DB & Scrapping') { 
                        ProductAddInStore(GetProduct,response);                        
                        flag++;
                        console.log('flag1', flag);
                        return { 'status':true, 'message': 'Product Found' };                        
                    } else {
...
async function ProductAddInStore  (GetProduct,response) {  
 // console.log('Product data postman raw - ', GetProduct)
        ...
        const allsku = [];
        var itemvariants = response.data.skuProperties[0].skuPropertyValues;
        await itemvariants.forEach(async function loop(itemsv)  {
            var createdjsondata = {
                "option1": itemsv.propertyValueDisplayName,
                "option2":itemsv.propertyValueName
            }
            allItems.push(createdjsondata)
        }); 
        ...
        // var itemskuList = response.data.skuList;
        // console.log("item sku List array ",  itemskuList)
        // await itemskuList.forEach(async function loop(itemsklist)  {
        //     var path = itemsklist.skuAttr.split("#");
        //     var path2 = path[1].split(";");
        //     var createdsku = {                
        //         "option1": path2[0],
        //         "option2":itemsklist.skuVal.actSkuCalPrice
        //     }
        //     allItems.push(createdsku)
        // });  
  ...
  router.post("/api/featchproduct", async (ctx) => {
   
    var GetPostdata = ctx.request.body.product.aliexpres

    GetPostdata.map(async (user) => {
        var Bulkdata = [];

        var path = user.link.split("/item/");
        var path2 = path[1].split(".");
        const exportpro_id =  path2[0];
        console.log("links get", exportpro_id)
        ...
        var config = {
            ...
            const shopifyStoresData = await axios(config)
      ...
      ctx.status = 200;
      ctx.body = shopifyStoresData;
    })
    


 /* Create custome code */
 router.post("/import-product", koaBody(), async (ctx) => {
